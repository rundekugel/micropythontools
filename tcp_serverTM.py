...
class _BaseTCPServer:
    ...
    def ListenIPAddress(self):
        return self._listenAddress or "0.0.0.0"
    ...
    def _RestartReceiveLoop(self):
        ...
        if self._timerParseReceive is None:
            ...
            self._timerParseReceive.periodic=0
    ...
    def _ParseReceiveData(self):
        ...
        if self._onReceiveCallback:
          if self.nsock and not self._clients: #work without clients list
            c=self.nsock
            try:
                data = c.recv(512)  # may fail due to socket closure
            except Exception as e:
                #self._lasterr = e
                if self.trace or b.errno!=110:
            ...
            if data:
                try:
                    ...
                    print(str(self)+' Callback Error:'+ str(e))
          else:                          
...
class SimpleTCPServer(_BaseTCPServer):
    def __init__(self, *args, **kwargs):
        ...
        if self._listenAddress:
            ...
            self._sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            #self._sock.bind((gethostname(), self._listenport))
            self._sock.bind((self._listenAddress, self._listenport))
        else:
            self._sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self._sock.bind(("0.0.0.0", self._listenport))
...
def cb(client,state):
  print("Client "+str(client)+" is "+str(state))
...
if __name__ == '__main__':
    ...
    server.onConnected = cb
    server.onDisconnected = cb
    ...
    #from tcp_serverTM.tcp_client import SimpleTCPClient
    import random

    clients = []
    for i in range(3):
        continue
        client = Client(1,gethostname(), 3888)
